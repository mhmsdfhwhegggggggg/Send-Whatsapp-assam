...
export const accountsTable = pgTable("accounts", {
  ...
  // proxy: optional per-account proxy, format: "http://user:pass@host:port"
  // Each account should ideally have its own residential proxy to avoid
  // WhatsApp detecting multiple sessions from the same IP.
  proxy:         text("proxy"),
...
export const settingsTable = pgTable("settings", {
  ...
  // Legacy single limit (kept for migration compatibility, not used by runner)
...
// Opt-out list: phones that replied STOP/إيقاف/etc.
...
// Log of all inbound messages received
